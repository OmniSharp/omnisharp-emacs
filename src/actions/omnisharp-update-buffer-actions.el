...
(defun omnisharp--before-change-function (beg end)
...
These information will be used by omnisharp--after-changed-function."
  (setq omnisharp-before-change-beg beg)
...
(defun omnisharp--after-change-function (begin end leng-before)
  ...
  (omnisharp--send-command-to-server-sync
   ...
   (let* ((filename-tmp (or buffer-file-name ""))
          ...
          (params `((StartLine   . ,(line-number-at-pos omnisharp-before-change-beg))
                    (EndLine     . ,(line-number-at-pos omnisharp-before-change-end))
                    (StartColumn . ,(omnisharp--column-from-pos omnisharp-before-change-beg))
                    (EndColumn   . ,(omnsiharp--column-from-pos omnisharp-before-change-end))
                    (NewText     . ,text))))
